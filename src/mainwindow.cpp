    ...
    , m_currentFile("")
{
    ...
    setWindowTitle(tr("grfx - Untitled"));
    resize(800, 600);
}
...
void MainWindow::newFromClipboard()
{
    ...
    if (canvas) {
        ...
        setCurrentFile("");
        setWindowTitle(tr("grfx - New from Clipboard"));
    }
}
...
void MainWindow::openFile()
{
    ...
    if (!fileName.isEmpty()) {
        if (m_tabWidget->loadDocument(fileName)) {
            ...
            QMessageBox::warning(this, "grfx", "Cannot open file " + fileName);
        }
    }
}
...
void MainWindow::saveFile()
{
    if (m_currentFile.isEmpty()) {
        ...
        if (!m_tabWidget->saveCurrentDocument(m_currentFile)) {
            QMessageBox::warning(this, "grfx", "Cannot save file " + m_currentFile);
        }
    }
}
...
void MainWindow::saveAsFile()
{
    ...
    if (!fileName.isEmpty()) {
        if (m_tabWidget->saveCurrentDocument(fileName)) {
            ...
            QMessageBox::warning(this, "grfx", "Cannot save file " + fileName);
        }
    }
}
...
void MainWindow::exportFile()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        if (canvas && canvas->saveCanvas(fileName)) {
            ...
            QMessageBox::warning(this, "grfx", "Cannot export file " + fileName);
        }
    }
}
...
void MainWindow::onTabChanged(int index)
{
    ...
    if (!currentTabName.isEmpty()) {
        setWindowTitle(QString("grfx - %1").arg(currentTabName));
    }
}
...
void MainWindow::connectCanvasSignals(Canvas *canvas)
{
    ...
    connect(canvas, &Canvas::documentModified, this, [this]() {
        ...
        if (canvas) {
            QString currentTabName = m_tabWidget->tabName(m_tabWidget->currentIndex());
            QString title = QString("grfx - %1%2").arg(currentTabName).arg(canvas->isModified() ? " *" : "");
            setWindowTitle(title);
        }
    });
